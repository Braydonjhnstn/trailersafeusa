...
function ProductsPage() {
  ...
  // Track size options based on selected model
  // This will be populated with actual data once you provide the track sizes
  const getTrackSizeOptions = (model) => {
    const trackSizeMap = {
    ...
    return trackSizeMap[model] || [];
  };

  const handleSearch = () => {
    // Filter products based on selected make and model
    // Track size will be determined from the model data you provide
    ...
    if (selectedMake) {
      ...
      filtered = filtered.filter(product => {
        ...
        return productTitle.includes(selectedMake.toLowerCase()) ||
               productTags.some(tag => tag.includes(selectedMake.toLowerCase()));
      });
    }
    ...
    if (selectedModel) {
      ...
      filtered = filtered.filter(product => {
        const productTitle = product.title.toLowerCase();
        const productTags = product.tags?.map(tag => tag.toLowerCase()) || [];
        return productTitle.includes(selectedModel.toLowerCase()) ||
               productTags.some(tag => tag.includes(selectedModel.toLowerCase()));
      });
      
      // If track sizes are provided for this model, filter by track size as well
      const trackSizes = getTrackSizeOptions(selectedModel);
      if (trackSizes.length > 0) {
        // Filter products that match any of the track sizes for this model
        filtered = filtered.filter(product => {
          const productTitle = product.title.toLowerCase();
          const productTags = product.tags?.map(tag => tag.toLowerCase()) || [];
          return trackSizes.some(size => 
            productTitle.includes(size.toLowerCase()) ||
            productTags.some(tag => tag.includes(size.toLowerCase()))
          );
        });
      }
    }
  ...
  // Use filtered products if available, otherwise show all products
